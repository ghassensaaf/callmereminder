...
/** @deprecated Use POST /api/vapi-configs. Only works when org has zero configs (bootstrap). */
router.put("/", requireAuth, requireOrg, requireOrgRole("owner", "admin"), async (req, res) => {
  try {
    const orgId = req.organizationId;
    const existing = await prisma.vapiConfig.count({ where: { organizationId: orgId } });
    if (existing > 0) {
...
router.delete("/", requireAuth, requireOrg, requireOrgRole("owner", "admin"), async (req, res) => {
  try {
    await prisma.vapiConfig.deleteMany({ where: { organizationId: req.organizationId } });
    res.json({
      vapiApiKeyDisplay: null,
      vapiPhoneNumberId: null,
      hasVapiKeys: false,
